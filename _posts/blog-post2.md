...
excerpt: >-
  Working through the first few exercises in Ethernaut.
---

I finished Crypto Zombies and figured I wanted to learn some more about Solidity, so I started on the Ethernaut series. Ethernaut is a great introduction to interacting with contracts.

This assumes that you have worked through the crypto zombies exercises, but that you don't know anything else about smart contract coding.

At the time of writing there are 11 levels.
...
And I love that they got the name from a weird comic from the 50s.

![Eternaut Comic](/images/2018-04-28-How-To-Hack-Ethereum-Contracts-The-Easy-Way/ethernaut.jpg)

## Hello Ethernaut

Hello Ethernaut is an opportunity to get acquainted with the game interface. You'll need to set up a [MetaMask](https://metamask.io) account, if you don't have one already.

Interacting with the level contract is pretty straightforward. The contract has a number of methods that you can call with:
...
I'm going to go through this level's contract with a fine tooth comb. If you just want to know how to complete the level, you can skip to [the solution here.](#the-solution) But if you want to grok solidity, you need to get your coding hands dirty.

### Checking the balance

Let's get started by checking the contract balance. We can do this with the command:
```javascript
...
The columns detail the various interface properties.Let's go through them one by one:
 ...
 - A true **constant** value indicates the method will *not* modify the blockchain. As a result you can `call` this method without using any gas. **constant** has already been deprecated in favour of **stateMutability**, detailed further down, but is still included for backward compatibility.
  - **type** indicates what type of interface it is. It can be a function, constructor, a fallback method (we'll come back to this
   later), or an event.
...
You pass it an address to `contributions` and it will return a javascript Big Number object:

I can see what my contributions and what the owners are with:
...
```javascript
(await contract.contributions(player)).toString(); // "0"
(await contract.contributions(await contract.owner())).toString(); "1e+21"
```
So basically we've found out that the owner has a lot more contributions than we do :D

### Wei, Gwei and Ether

It's important to note that the owner's 10^21^ contributions are in wei. There are **10^18^**, or one **quintillion**, wei in one ether. You could also say that an ether is one Exawei (as in Exabyte).

Note that gas price is typically in Gwei (Gigawei), which is 10^9 wei, or a **billion** wei.
...
- `this` : the current contract.(http://solidity.readthedocs.io/en/v0.4.23/types.html#address)
- `balance` : the property of an address (in Wei).

We can't use this at the moment, since we aren't the owner, so let's move on!

### The fallback method

Finally we get to the fallback method! So what is a fallback method?
...
To summarize, first of all we send a contribution using the `contribute()` method chained with `sendTransaction()` so that we can specify the amount.

```javascript
await contract.contribute.sendTransaction({value: 1});
```
Methods that are payable are called with a `send()` method (In this case, it is `sendTransaction()` instead of `send()`). Non-payable methods are called with a `call()` method. 

Now that our `contributions[msg.sender]` is greater than zero, we can invoke the fallback method by sending the contract an amount:
```javascript
await contract.sendTransaction({value: 1});
```
